...
| `public/assets/hero-bg.mp4` · `hero-bg-mobile.mp4` | 히어로 배경 영상(**사운드 포함**). 1920×1080 11MB / 608×1080 4.9MB(≤760px) |
...
| `src/ProjectsContext.jsx` | projects+categories 전역 제공, **`catLabel`**(카테고리 라벨은 반드시 이걸 쓸 것), **`trusted`**(noindex 판정용 — 7절 (1)) |
...
| `duo` | **좌우 미디어(2개 나란히)** | left{media,ar,loop}, right{…}, caption | 정사각·세로 소재용. 아래 설명 참고 |
| `divider` | 구분선 | — | 이 지점에서 스크롤 효과 구역이 나뉨 |

### duo(좌우 미디어) 배치 규칙 (2026-09-22)

왼쪽·오른쪽에 영상이나 이미지를 하나씩. 업로드한 파일만 넣을 수 있다(유튜브는 자체
크기 규칙이 있어 옆 칸과 높이를 맞출 수 없다 → 기존 `video` 블록을 쓸 것).

⚠️ **폭 비율을 고정하지 않는다.** 고정하면 세로 소재에서 잘라내기(cover)나
여백(contain) 중 하나를 골라야 한다. 대신 **각 소재의 가로세로비(`ar`)에 비례해**
폭을 나눈다(`flex-basis:0` + `flex-grow:ar`) — 두 쪽 높이가 저절로 같아진다.

`ar`은 **업로드 시점에 재서 블록에 저장한다.** 나중에 로드 후 재면 레이아웃이 한 번 튄다.

실측(본문폭 1240):
```
1:1 + 16:9   446x446 / 792x446     9:16 + 16:9  298x529 / 940x529 (왼쪽 24%)
2:1 + 3:1    495x248 / 743x248     4:3 + 9:16   871x653 / 367x653
```
한쪽이 15% 아래로 내려가는 조합(세로+초광폭 4%)과 모바일(≤720px)은 위아래로 쌓고,
쌓일 땐 80vh로 높이를 제한한다.

⚠️ 기존 `image` 블록의 type을 `duo`로 바꾸면 옛 `media` 값이 남아 있지만 화면에는
쓰이지 않는다. 점검 스크립트를 짤 때 `b.media`만 보면 duo의 `left`/`right`를 놓친다
(2026-09-22에 그렇게 잘못 읽어 "저장 안 됨 / 고아 38MB"라고 오보한 적 있음).
**파일 참조 검사는 DB 전체를 문자열로 만들어 파일명 포함 여부로 판정할 것.**

### features(특징 카드) 배치 규칙 (2026-09-21)

한 줄 최대 3칸, **단 한 칸만 남는 줄은 만들지 않는다.** 규칙은 `WorkDetail.jsx`의
`featureRows()`에 있다.
...
칸 폭은 개수와 무관하게 `--card`(=`clamp(200px,28vw,360px)`)로 **전부 같다.**
줄은 각각 가운데 정렬된다.

⚠️ **테두리·배경은 바깥 상자가 아니라 `.wb-feat-row`(줄)에 준다.** 그리드로 두거나
바깥 상자 하나로 두르면 짧은 줄의 남는 자리까지 상자 안에 들어가, 끝 칸이 테두리까지
이어진 것처럼 보여 **칸 크기가 달라 보인다**(사용자가 실제로 지적한 지점).
줄이 맞닿는 곳은 `margin-top:-1px`로 겹친 선을 지우고, 첫 줄 위·마지막 줄 아래만
모서리를 둥글린다.
...
## 7. 가장 최근 작업 (2026-09-01) ✅ 완료·라이브 검증됨

**(1) 🔴 프로젝트 페이지에 `noindex`가 잘못 붙던 버그** (커밋 `2da506a`) — 오늘의 핵심

- 구글 서치 콘솔: `'NOINDEX' 태그에 의해 제외되었습니다 — 웹사이트 — 2페이지`.
  **사이트맵에 "색인해달라"고 내면서 페이지에선 "빼달라"고 말하고 있었다.**
- **원인**: DB 조회가 실패하면 `ProjectsContext`가 `SEED_PROJECTS`로 폴백하는데,
  **시드 6건은 현재 DB 10건과 슬러그가 하나도 겹치지 않는다.** 그 상태에서
  `WorkDetail`의 `notFound = !loading && !p`가 참이 되어 **멀쩡한 프로젝트 페이지
  전부에 `noindex`**가 붙는다. 구글이 하필 그 순간 렌더하면 색인에서 빠진다.
- **수정**: `ProjectsContext`에 `trusted`(= `isConfigured && !error && projects !== null`)를
  노출하고 `notFound = !loading && trusted && !p`로 바꿨다.
- ⚠️ **`trusted`를 빼지 말 것.** 데이터를 못 믿는 상황에서 "삭제된 프로젝트"라고
  단정하면 안 된다. **일시적 소프트 404가 색인되는 편이, 멀쩡한 페이지가 빠지는
  것보다 훨씬 낫다.**
- ⚠️ **시드(`SEED_PROJECTS`)가 완전히 낡았다.** DB와 슬러그가 0건 일치.
  DB 장애 시 방문자는 존재하지 않는 프로젝트 6건을 보게 된다. 폴백을 없애든
  갱신하든 정리가 필요하다(미결).
- 검증: 정상 로드 `index, follow` / 없는 slug는 여전히 `noindex, follow`(회귀 없음).

**(2) WORK 상세에 '다른 프로젝트' 3건** (커밋 `1e50a17` 계열)

- 내부 링크가 별 모양이었다. `/work` → 상세 → 다시 `/work`. **각 상세가 받는 내부
  링크가 1개뿐**이라 검색엔진이 중요도를 낮게 본다(미색인 7건의 사유
  `발견됨 - 현재 색인이 생성되지 않음`과 직결).
- 상세 하단에 3건을 붙여 **1개 → 4개**로 늘렸다. 같은 분야 우선, 모자라면 나머지.
- ⚠️ 선택 규칙은 `seoRoutes.js`의 `pickRelated`에 두고 **프리렌더와 공유**한다.
  화면과 크롤러의 목록이 다르면 클로킹이다. `prerender.mjs`의 noscript에도 같은
  링크가 들어간다 — 없으면 Yeti는 상세끼리 이동할 길이 없다.

**(3) 새 프로젝트(강화성당) 반영** (커밋 `8b01a4d`)

- 관리자에서 추가했는데 사이트맵이 13개에 머물러 있었다. **DB만 바뀌면 재배포
  전까지 사이트맵·크롤러용 HTML은 옛날 것**이다(7-1절 (1) 참고).
- 코드 변경 없이 **빈 커밋**으로 빌드를 트리거했다. Vercel `Redeploy`도 같은 효과.
  ```bash
  git commit --allow-empty -m "빌드 트리거: …"
  ```
- 결과: 사이트맵 13 → **14**, 14/14 응답 200, 새 페이지 본문 1,209자.

**(4) SNS 계정 정비**

- 비핸스 `virenmotion` → **`viren2`**, 링크드인 `viren-creative-871663405` → **`virenmotion`**.
  ⚠️ **두 곳을 함께 고쳐야 한다** — `socials.jsx`(화면)와 `index.html`의 JSON-LD `sameAs`.
  구조화 데이터가 화면과 어긋나면 구글이 회사 계정을 잘못 연결한다.
- **한글 검색 노출의 조건**: 프로필 안에 `바이렌`이 있어야 한글 검색에 매칭된다.
  실측(og 태그·본문 기준) — YouTube 80회 ✅ / Behance 24회 ✅(사용자가 이름을
  `바이렌 VIREN`으로 수정) / Instagram·LinkedIn은 봇 차단이라 외부 확인 불가.
- 링크드인은 `/in/`(개인 프로필)이다. 회사 브랜드 검색에는 `/company/`(회사 페이지)가
  유리하다 — 만들면 주소 교체 필요(미결).

**(5) CAREER 버튼 정리** (커밋 `c6692f2`)

- 지원서 양식 다운로드를 **왼쪽 노란 박스(검은 글씨)**, 지원하기를 **오른쪽 끝**으로.
- 호버는 색 대신 `opacity` — 배경이 채워져 있어 색 변화가 안 보인다.
- ⚠️ 520px 이하에서는 `.job-apply`의 `margin-left:auto`를 해제한다. 줄이 바뀌면
  auto 여백이 어색해진다.

## 7-1. 2026-08-18 작업 ✅ 배포됨
...
- **화면별 소스** — `matchMedia('(max-width:760px)')`로 **마운트 시 한 번만** 고른다.
  ...
  | 화면 | 파일 | 해상도 | 용량 | 가로 잘림 |
  |---|---|---|---|---|
  | 모바일 ≤760 | `hero-bg-mobile.mp4` | 608×1080 | 4.9MB | 18% |
  | 태블릿·데스크톱 | `hero-bg.mp4` | 1920×1080 | 11MB | 59% / — |

  (용량은 사운드 포함. 무음이던 시절엔 4.0MB / 9.7MB였다 — 아래 "사운드" 참고)
...
- 처음 압축할 때 `-an`으로 오디오를 뺐다가 원본에서 되살렸다(AAC 128k 스테레오).
  **재인코딩할 일이 있으면 `-an`을 넣지 말 것.** 원본에는 오디오가 있다.
- ⚠️ **브라우저는 소리 있는 자동재생을 차단한다.** 설정이 아니라 하드 제한이라 무시하면
  `play()`가 거부되고 **영상 자체가 재생되지 않는다.** 그래서 3단계로 처리한다.
  1. 소리를 켠 채로 `play()` 시도 — 미디어 engagement가 쌓인 재방문자는 통과한다.
  2. 막히면 음소거로 재생해두고 첫 상호작용(클릭·키·스크롤·터치·휠)에 해제한다.
  3. `.sound-toggle` 버튼(헤더 메뉴 아래 우측)으로 언제든 켜고 끈다.
- **버튼을 한 번 누르면 `userChose`가 서고 2번이 개입하지 않는다.** 사용자가 끈 소리를
  스크롤이 다시 켜는 일을 막는 장치다. 지우지 말 것.
- **볼륨은 배경 불투명도를 따라간다** — 히어로 1.0 → ABOUT 0.2 → WHAT WE DO 이후 0.
  화면에서 사라진 배경이 소리만 내는 상황을 막는다. 버튼도 같은 구간에서 숨는다.
- ⚠️ **버튼은 `createPortal`로 body에 띄운다.** `.backdrop`은 `z-index:0`이라 그 안에 두면
  `z-index:1`인 섹션에 가려 눌리지 않는다(`main`이 `z-index:2`로 stacking context를 만든다).
- ⚠️ **모바일 헤더는 84px로 데스크톱(75px)보다 높다.** 버튼 `top`을 공유하면 겹친다.
...
### 🔜 다음에 이어서 할 것
1. **구글 `NOINDEX` 검증 결과 확인 (2026-09-08~15경)** — 7절 (1) 수정 후
   서치 콘솔에서 `수정사항 검증`을 시작했다(상태 `시작됨`). 통과하면 2페이지가
   색인에 들어온다. ⚠️ 검증 중에 재검증을 반복해 누르면 순번이 뒤로 간다.
2. **구글 색인 수 추이** — 현재 **색인 4 / 미색인 9**(`NOINDEX` 2 + `발견됨` 7).
   `발견됨` 7건은 우리가 고칠 게 아니라 사이트 신뢰도 문제다. 오늘 넣은 내부
   링크(7절 (2))가 여기에 작용하므로 2주 뒤 숫자가 움직였는지 볼 것.
3. **`SEED_PROJECTS` 정리** — DB와 슬러그 0건 일치. DB 장애 시 존재하지 않는
   프로젝트 6건이 노출된다. 폴백을 없애거나 갱신할 것(7절 (1) 참고).
4. **충주댐 물문화관 본문 353자** — 프로젝트 10건 중 가장 얇다. 다른 프로젝트는
   739~2,282자. 얇은 페이지는 색인이 잘 안 된다.
5. **링크드인 회사 페이지(`/company/`)** — 지금은 개인 프로필(`/in/`)이다.
   만들면 `socials.jsx` + `index.html`의 `sameAs` 두 곳을 교체할 것.
6. **비핸스 ABOUT 첫 문장** — `VIREN은 …` → `바이렌(VIREN)은 …` 으로 바꾸면
   본문에서도 한글 매칭이 된다(이름은 이미 `바이렌 VIREN`으로 수정됨).
7. **기존 비핸스 계정(`behance.net/virenmotion`) 삭제** — 구글 재크롤링 후에.
8. **강화성당 프로젝트 영상** — 사용자가 나중에 추가 예정. 추가 후 **재배포 필요**
   (캡션이 크롤러용 HTML에 들어간다).

### 완료됨 (2026-09-01)
- 프로젝트 페이지 `noindex` 버그 수정 + 구글 검증 시작.
- WORK 상세에 '다른 프로젝트' 3건 — 내부 링크 1개 → 4개.
- 강화성당 프로젝트 반영(사이트맵 13 → 14, 14/14 응답 200).
- 비핸스 `viren2` · 링크드인 `virenmotion`으로 교체(화면 + `sameAs`).
- CAREER 지원서 다운로드 노란 박스 + 지원하기 우측 정렬.

### 검색 노출 현황 (2026-09-01)
| | 상태 |
|---|---|
| **네이버 색인** | 홈 ✅ / 강화성당(신규) ✅ — "문서가 성공적으로 색인됐습니다" |
| 네이버 노출(30일) | 노출 55 · 클릭 7 · CTR 12.7%. 키워드는 `viren`, `바이렌 모션` |
| **구글 색인** | **4 / 14** (미색인 9: `NOINDEX` 2 + `발견됨` 7) |
| 구글 사이트맵 | 14개 발견, 처리 완료(26.9.1 읽음) |

- ⚠️ **`바이렌` 단독 검색은 당분간 어렵다.** 네이버는 주방용품·청소기 브랜드,
  구글은 중국 반도체 기업이 선점했다. 기술로 넘을 벽이 아니다.
  현실적 목표는 `viren`, `바이렌 모션`, `바이렌 미디어파사드` 같은 조합 키워드.
- 사용자가 **불가능하다고 밝힌 것**: 발주처·보도자료 백링크 확보, 네이버 블로그 운영.
  이 둘이 가장 효과가 큰 수단이었으므로, 남은 건 사이트 내부 개선뿐이다.
...
- ⚠️ **저장소를 열 때는 `C:\viren` 을 쓴다** (2026-09-17 설정). 실제 위치는 Dropbox 안이고,
  ...
  - `.claude/launch.json`이 `--prefix C:\viren` 으로 서버를 띄우므로, **어느 경로에서 열어도**
    dev 서버는 공백 없는 경로에서 실행된다(사용자가 신경 쓸 것 없음). 그래도 안 되면 `C:\viren`에서 세션을 열 것.
...
- **없는 주소는 자동으로 `noindex` 처리된다.** SPA라 서버가 어떤 경로든 200을 반환해 소프트 404가 되기 때문.
  삭제된 프로젝트는 `WorkDetail.jsx`의 `notFound`, 그 외 경로는 `App.jsx`의
  `path="*"` → `NotFoundPage`가 담당한다.
- ⚠️⚠️ **`notFound` 조건을 함부로 줄이지 말 것 — 두 항목 모두 사고로 추가됐다.**
  `notFound = !loading && trusted && !p`
  - `!loading` 없으면: 데이터 도착 전에 정상 페이지가 noindex로 스냅샷된다.
  - `trusted` 없으면: DB 조회 실패 → 시드 폴백 → **프로젝트 페이지 전부 noindex**.
    2026-09-01에 실제로 발생해 구글이 2페이지를 뺐다(7절 (1)).
- ⚠️ **`/about` 주소는 존재하지 않는다.** ABOUT은 홈의 한 구간(`fp-panel`)이라 라우트가 없다.
  검색엔진 수집 요청에 `/about`을 넣지 말 것 — 실제로 한 번 잘못 제출했다. 유효한 주소는 사이트맵에 있는 것이 전부다(현재 14개).
...
### 고아 파일 정리 — 관리자 「저장공간」 탭 (2026-09-22 신설)

`/admin` → **저장공간** 탭. 전체/사용중/고아 용량을 보여주고 버튼 한 번으로 지운다.
구현은 `src/lib/storageAudit.js`.

- 참조 검사는 칼럼을 지정하지 않고 `projects`·`jobs`·`site_settings` **전체를 문자열로
  만들어** 파일명 포함 여부로 판정한다. 미디어 칼럼이 늘어도(duo의 left/right 등) 따라간다.
- ⚠️ **삭제 성공은 응답이 아니라 "다시 센 파일 개수"로 판정한다.** 권한이 없으면 Supabase가
  오류가 아니라 **200 + 빈 배열**을 돌려줘 지운 것처럼 보인다. 읽기 전용 anon 키로는
  지워지지 않으므로 **Claude는 이 정리를 대신 못 한다** — 로그인한 사용자가 눌러야 한다.
- 삭제 직전에 목록을 다시 대조하고, 그 사이 바뀌었으면 중단한다.

실적(2026-09-22): 파일 128 → 98개, 191.3MB → 149.7MB.
