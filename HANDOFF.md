...
7. **기존 비핸스 계정(`behance.net/virenmotion`) 삭제** — 사이트 링크와 JSON-LD의 `sameAs`는
   이미 `viren2`로 바꿨다(2026-08-18). 기존 계정은 프로젝트 0개·팔로워 1명이라 잃을 게 없다.
   ⚠️ **구글이 홈페이지를 다시 크롤링한 뒤에 지울 것.** 아직 구글이 옛 계정을 회사 계정으로
   알고 있는데 먼저 지우면 그 자리가 404가 되고 새 계정 인식이 늦어진다.
   삭제 경로: 해당 계정 로그인 → `behance.net/settings` 맨 아래 → Delete Account.

### 완료됨 (2026-08-18)
- **비핸스 링크를 `viren2`로 교체** — `socials.jsx`(화면)와 `index.html`의 JSON-LD `sameAs`
  두 곳을 함께 고쳤다. 구조화 데이터가 화면 링크와 어긋나면 구글이 계정을 잘못 연결한다.
- **히어로 배경 사운드 복원 + 토글 버튼**. 압축 때 -an으로 빠졌던 오디오를 되살리고,
  브라우저의 자동재생 차단에 3단계로 대응(자동 시도 → 첫 상호작용 → 버튼). 상세는 7절 (3).
