...
### 🔜 내일(2026-08-11) 이어서 할 것
1. **산출물 수량 확정** — 사용자가 대표님과 조율 예정. 사이트 WORK의 `산출물` 표기와
   유튜브 설명문(스펙 표 기준)의 개수가 8건 어긋나 있음. 상세 대조표는
   `VIREN_유튜브_설명문.md` 맨 아래 "웹사이트 ↔ 스펙 표 대조" 참고.
   - 예: 충주댐 사이트 6종 / 표 10종, 경주 4종 / 7종, 청주 14종 / 16종
   - 사이트는 요약, 유튜브는 전체 내역이라 **완전히 같을 필요는 없으나 총 개수는 맞출 것**.
   - 확정되면 설명문 파일 반영 필요.
2. **충주 물문화관 봄시즌 프로젝트 미등록** — 등록하면 `sitemap.xml` 갱신 필요(현재 12 URL).
   스펙: 1920×1200, 봄 시즌 이벤트 영상 1종(4분 이내), 한국수자원공사, 2026.
3. **유튜브 채널 설정** — 10-1절의 ①~④(채널명·설명·키워드·영상 제목) 사용자 미적용 상태.
4. 신규 프로젝트(몬순) 구글 색인 요청 미완료.

### 상태
- 오늘 커밋 8건 전부 배포·라이브 검증 완료. 작업 트리 깨끗(`showreel-build/`만 untracked).
